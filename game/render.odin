...
render :: proc(state: ^State) {
	...
	if linalg.length(state.player.vel) > 0 {
		rl.DrawLineEx(project(rl.Vector3(0), state), project(state.player.vel, state), 4, rl.PINK)
	}

	rl.DrawLineEx(project(rl.Vector3(0), state), project(state.player.move_dir, state), 4, rl.BLUE)
